...
describe("Keystore", (): void => {
  ...
  const tests_spec: any = [
    ...
    ["createUser", () => keystore.createUser(username1, password), (x: boolean) => x, Matcher.toBe, () => true],
    ["createRepeatedUser", ()=>keystore.createUser(username1, password),  (x)=>x,  Matcher.toThrow,   ()=>"user already exists: " + username1],
    ["listUsers", () => keystore.listUsers(), (x: string[]) => x, Matcher.toContain, () => [username1]],
    ["exportUser", () => keystore.exportUser(username1, password), (x: string) => x, Matcher.toMatch, () => /\w{78}/],
    ["getExportedUser", () => keystore.exportUser(username1, password), (x: string) => x, Matcher.Get, () => exportedUser],
    ["importUser",         ()=>keystore.importUser(username2, exportedUser.value, password),
      (x: boolean) => x, Matcher.toBe, () => true],
    ...
    ["deleteUser1", () => keystore.deleteUser(username1, password), (x: boolean) => x, Matcher.toBe, () => true],
    ["deleteUser2", () => keystore.deleteUser(username2, password), (x: boolean) => x, Matcher.toBe, () => true],
    ["deleteUser3", () => keystore.deleteUser(username3, password), (x: boolean) => x, Matcher.toBe, () => true],
  ]
