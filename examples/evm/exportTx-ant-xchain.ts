...
const xChainBlockchainIdStr: string =
  Defaults.network[networkID].X.blockchainID;
const xChainBlockchainIdBuf: Buffer = bintools.cb58Decode(
  xChainBlockchainIdStr
);
const cChainBlockchainIdStr: string =
  Defaults.network[networkID].C.blockchainID;
const cChainBlockchainIdBuf: Buffer = bintools.cb58Decode(
  cChainBlockchainIdStr
);
...
const main = async (): Promise<any> => {
  ...
  transferableOutput = new TransferableOutput(
    antAssetIDBuf,
    secpTransferOutput
  );
  exportedOuts.push(transferableOutput);
