 ...
 export class UTXOSet extends StandardUTXOSet<UTXO>{
   ...
   getMinimumSpendable = (
     ...
     for(let i: number = 0; i < utxoArray.length && !aad.canComplete(); i++) {
       ...
       if(u.getOutput() instanceof AmountOutput && aad.assetExists(assetKey) 
   ...
   buildBaseTx = (
     ...
     if(threshold > toAddresses.length) {
       /* istanbul ignore next */
       throw new Error(`Error - UTXOSet.buildBaseTx: threshold, ${threshold}, is greater than number of addresses`);
     }
   ...
   buildCreateAssetTx = (
     ...
     let CAtx: CreateAssetTx = new CreateAssetTx(
       networkID, 
       blockchainID, outs, ins, memo, name, symbol, denomination, initialState);
