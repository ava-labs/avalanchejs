...
export abstract class ValidatorTx extends BaseTx {
   ...
   * Class representing an unsigned AddSubnetValidatorTx transaction.
      ...
      * @param nodeID Optional. The nodeID of the validator being added.
  ...
  */
  ...
  serialize(encoding: SerializedEncoding = "hex"): object {
    ...
    return  {
      ...fields,
      "stakeOuts": this.stakeOuts.map((s: TransferableOutput) => s.serialize(encoding)),
  ...
  getStakeOutsTotal(): BN {
    let val: BN = new BN(0);
    this.stakeOuts.forEach((stakeOut: TransferableOutput) => {
      val = val.add((stakeOut.getOutput() as AmountOutput).getAmount());
    });
    return val;
  }
  ...
  fromBuffer(bytes: Buffer, offset: number = 0): number {
    offset = super.fromBuffer(bytes, offset);
    const numstakeouts: Buffer = bintools.copyFrom(bytes, offset, offset + 4);
    offset += 4;
  ...
  toBuffer(): Buffer {
      ...
      this.stakeOuts.forEach((stakeOut: TransferableOutput) => {
        let out: Buffer = stakeOut.toBuffer();
        barr.push(out);
        bsize += out.length;
      });
    ...
    delegationFee: number = undefined
      ...
      if(typeof delegationFee === "number") {
          if(delegationFee >= 0 && delegationFee <= 100) {
              ...
              throw new Error(`AddValidatorTx.constructor -- delegationFee, ${delegationFee}, must be in the range of 0 and 100, inclusively.`);
          }
      }
  }
}