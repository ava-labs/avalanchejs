...
function createRequest(config: AxiosRequestConfig): Request {
    ...
    if (config.auth) {
        ...
        const password = config.auth.password ? encodeURIComponent(config.auth.password) : '';
...
export default async function fetchAdapter (config: AxiosRequestConfig): Promise<AxiosResponse> {
    ...
    return new Promise((resolve, reject) => {
        if (response instanceof Error) {
            ...
            if(!response.status || !response.config.validateStatus || response.config.validateStatus(response.status)){
                resolve(response);
            } else {
                const error: AxiosError = {
