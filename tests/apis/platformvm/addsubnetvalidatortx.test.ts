...
import { AddSubnetValidatorTx, PlatformVMConstants, SubnetAuth } from "src/apis/platformvm"
...
describe("AddSubnetValidatorTx", (): void => {
  ...
  const addSubnetValidatorTxBuf: Buffer = Buffer.from(addSubnetValidatorTxHex, "hex")
  ...
  test("getTypeName", async (): Promise<void> => {
    const addSubnetValidatorTxTypeName: string = addSubnetValidatorTx.getTypeName()
  ...
  test("getTypeID", async (): Promise<void> => {
    ...
    expect(addSubnetValidatorTxTypeID).toBe(PlatformVMConstants.ADDSUBNETVALIDATORTX)
  })
