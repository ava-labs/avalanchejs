...
describe('Mnemonic', () => {
  test('vectors', (): void => {
    vectors.forEach(async (vector: Vector, index: number): Promise<void> => {
      ...
      const entropyToMnemonic: string = mnemonic.entropyToMnemonic(vector.entropy, wordlist)
      expect(vector.mnemonic).toBe(entropyToMnemonic)
      const mnemonicToEntropy: string = mnemonic.mnemonicToEntropy(vector.mnemonic, wordlist)
      expect(mnemonicToEntropy).toBe(vector.entropy)
      const password: string = "TREZOR"
      const mnemonicToSeed: Buffer = await mnemonic.mnemonicToSeed(vector.mnemonic, password)
  ...
  test('badMnemonics', (): void => {
    ...
    badMnemonics.forEach((badMnemonic: BadMnemonic, index: number): void => {
      const validateMnemonic: string = mnemonic.validateMnemonic(badMnemonic.mnemonic, wordlist)
  ...
  test('malformedMnemonics', (): void => {
    const wordlist = mnemonic.getWordlists("english") as string[]
    malformedMnemonics.forEach((malformedMnemonic: string, index: number): void => {
      const validateMnemonic: string = mnemonic.validateMnemonic(malformedMnemonic, wordlist)
      expect(validateMnemonic).toBeFalsy()
    })
  })
  ...
  test('test mnemonic lengths', (): void => {
    ...
    let mnemnnic: string = mnemonic.generateMnemonic(strength, randomBytes, wordlist)
